...
class MAC_address:
    ...
    def __init__(self, address=None):
        assert(isinstance(address, (type(None), bytes))),\
            f"Address should be bytes type: {address}"
...
class ARP:
    ...
    def __str__(self):
        if self.operation == ARP_REQUEST:
            return f"ARP Request: Who has {self.dst}? Tell {self.src}"

        if self.operation == ARP_REPLY:
            return f"ARP Reply:   {self.src} is at {self.hw_src}"
...
if len(sys.argv) != 4:
    print(f"usage: {sys.argv[0]} <local_ip> <remote_ip> <iface>")
    sys.exit()
